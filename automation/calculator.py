...
class SprintCalculator:
    ...
    def calculate_cmmi_measures(self,
        ...
        # 1. Completion Rate
        planned_sp = sprint_metadata.get('planned_sp') or 0  
        actual_sp = sprint_metadata.get('actual_sp') or 0
        cmmi['completion_rate'] = (
            actual_sp / planned_sp if planned_sp > 0 else 0
        )
        ...
        actual_dev = sprint_metadata.get('actual_dev_effort') or 0
        estimated_dev = sprint_metadata.get('estimated_dev_effort') or 0
        cmmi['effort_estimation_accuracy'] = (
            estimated_dev / actual_dev if actual_dev > 0 else 0
        )
        ...
        bug_fixing = sprint_metadata.get('bug_fixing_effort') or 0
        cmmi['bug_fixing_effort_pct'] = (
            bug_fixing / actual_dev if actual_dev > 0 else 0
        )
        
        # 4. Utilization Rate
        sum_actual = sprint_metadata.get('sum_actual_hours') or 0
        sum_working = sprint_metadata.get('sum_working_hours') or 0
        cmmi['utilization_rate'] = (
            sum_actual / sum_working if sum_working > 0 else 0
        )
        
        # 5. CMMI Productivity
        actual_sp = sprint_metadata.get('actual_sp') or 0
        if cmmi['utilization_rate'] > 0:
            cmmi['cmmi_productivity'] = actual_sp / cmmi['utilization_rate']
        else:
